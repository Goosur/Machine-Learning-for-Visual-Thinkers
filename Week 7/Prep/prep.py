...
# Devon Gardner
# 09/08/2020
#   python prep.py -i <input_file> -x <x_header or x_col> -y <y_header or y_col> or py prep.py -i <input_file> -x <x_header or x_col> -y <y_header or y_col>
...
def regression(degree, X, Y):
    ...
    fig.suptitle('Single Model of D={0}'.format(degree))
    ax[0].plot(X, Y, 'ob')
    ...
    ax[0].plot( X_synth, Y_synth, '-r', label="D={0}".format(d))
...
def visualize(file_name, x_col, y_col):
    ...
    # Visualize data.
    # heatmap(data.cov(), 'Covariance', 'Covariance')
    # heatmap(data_norm_range.cov(), 'Norm Range Covariance', 'Covariance')
    # heatmap(data_norm_z.cov(), 'Norm Z Covariance', 'Covariance')

    scatter(X, Y, file_name)
    scatter(X_norm_z, Y_norm_z, file_name + ' norm z')
...
def main(argv):
    ...
    try:
        ...
        print('usage: prep.py -i <input_file> -x <x_header or x_col> -y <y_header or y_col>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print('usage: prep.py -i <input_file> -x <x_header or x_col> -y <y_header or x_col>')
            sys.exit()
        elif opt == '-i':
            input_file = arg
        elif opt == '-x':
